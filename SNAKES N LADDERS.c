...
#define handle_var GetStdHandle(STD_OUTPUT_HANDLE) //Note: Here,HANDLE is used to input specific modes for graphical representations on console
...
void welcome_screen(){
	...
	SetConsoleTextAttribute(handle_var,FOREGROUND_LIGHTBLUE);                     
	gotoxy(46,25);
	printf("Programmed By 'ATS Group'");
	SetConsoleTextAttribute(handle_var,FOREGROUND_RED);
	gotoxy(46,26);
	printf("Abdul Basit Akhtar(19k-1376)");
    SetConsoleTextAttribute(handle_var,FOREGROUND_YELLOW);
	gotoxy(46,27);
	printf("Taha Yousuf(19k-1352)");
    SetConsoleTextAttribute(handle_var,FOREGROUND_GREEN);
	gotoxy(46,28);
	printf("Shahzaib Khan(19k-0273)");
...
int new_game(){
	...
	while(1){
	    ...
	    SetConsoleTextAttribute(handle_var,FOREGROUND_WHITE);
		printf("\n\nEnter 1st player's name (limit: 18 characters): ");
	    scanf("%s",p_info.p1_name); // i.e. can't use %[^\n] , gives error in filing
	    printf("Enter 2nd player's name (limit: 18 characters): ");
	    scanf("%s",p_info.p2_name);
	 	if (strcmp(p_info.p1_name,p_info.p2_name)) //i.e to avoid same names, Note: strcmp() returns '0' for same & '1' for varying  
		break;
		printf("\a");
	}
...
void rename_data_files(){
	SetConsoleTextAttribute(handle_var,FOREGROUND_RED);
	printf("\nWarning: Application will malfunction after restart,if the file names aren't changed back manually!\a");
	SetConsoleTextAttribute(handle_var,FOREGROUND_LIGHTBLUE);
...
void hidden_settings(){
	while(1){
		...
		switch(cursor_ptr){
			...
			SetConsoleTextAttribute(handle_var,FOREGROUND_WHITE);
			scanf("%c",&choice);
			if ((choice=='Y'||choice=='y')&&!sound_flag)
			sound_flag=1;
			else if((choice=='Y'||choice=='y')&&sound_flag)
			sound_flag=0;
			break;
			default:
			printf("\nError!\a");
...
void save_game(){
	...
	while(X!=escape){
		...
		printf("Note: Same 'names' will not be accepted!\n");
		SetConsoleTextAttribute(handle_var,FOREGROUND_LIGHTBLUE);
		printf("Enter the authentication name for current game (limit: 18 characters): ");
    	SetConsoleTextAttribute(handle_var,FOREGROUND_WHITE);
